...
#undef OFFSET
	...
	ReloadCBRace();
	stAttack = env->addStaticText(gDataManager->GetSysString(3316).data(), Scale(205, 34, 235, 54), false, false, wFilter);
	defaultStrings.emplace_back(stAttack, 3316);
	ebAttack = AlignElementWithParent(env->addEditBox(L"", Scale(240, 26, 289, 46), true, wFilter, EDITBOX_ATTACK));
	ebAttack->setTextAlignment(irr::gui::EGUIA_CENTER, irr::gui::EGUIA_CENTER);
	stDefense = env->addStaticText(gDataManager->GetSysString(1323).data(), Scale(276, 34, 299, 50), false, false, wFilter);
	defaultStrings.emplace_back(stDefense, 1323);
	stDefense->setVisible(false);
	ebDefense = AlignElementWithParent(env->addEditBox(L"", Scale(293, 26, 344, 46), true, wFilter, EDITBOX_DEFENSE));

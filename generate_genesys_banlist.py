...
OUTPUT_BANLIST_PATH = "genesys.lflist.conf"
...
def get_link_and_pendulum_cards(conn):
	cursor = conn.cursor()
	pendulum_flag = 0x001000000
	link_flag = 0x4000000
	flag = link_flag | pendulum_flag
	cursor.execute("""
				   SELECT datas.id
				   FROM datas
				   WHERE (datas.type & ?) != 0
				   """, (flag,))
	result = [found[0] for found in cursor.fetchall()]
	return result
...
def generate_banlist(cards, db_path, output_path):
	...
	# ban every link and pend
	for card_id in get_link_and_pendulum_cards(conn):
		banlist_entries.append(f"{card_id} 0 0")
